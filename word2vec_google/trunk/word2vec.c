...
long long train_words = 0, word_count_actual = 0, trainingIterations = 5, file_size = 0, classes = 0;
...
void *TrainModelThread(void *id) {
  ...
  long long l1, l2, c, target, label, local_iter = trainingIterations;
  ...
  while (1) {
    //word_count and last_word_count gets initialized to 0
    if (word_count - last_word_count > 10000) {
      ...
      if ((debug_mode > 1)) {
        ...
        printf("%cAlpha: %f  Progress: %.2f%%  Words/thread/sec: %.2fk  ", 13, alpha,
         word_count_actual / (real)(trainingIterations * train_words + 1) * 100,
         word_count_actual / ((real)(now - start + 1) / (real)CLOCKS_PER_SEC * 1000));
        fflush(stdout);
      }
      alpha = starting_alpha * (1 - word_count_actual / (real)(trainingIterations * train_words + 1));
...
void TrainModel() {
  ...
  if (classes == 0) {
    ...
    int clcn = classes, trainingIterations = 10, closeid;
...
int main(int argc, char **argv) {
  ...
  if ((i = ArgPos((char *)"-trainingIterations", argc, argv)) > 0) iter = atoi(argv[i + 1]);
